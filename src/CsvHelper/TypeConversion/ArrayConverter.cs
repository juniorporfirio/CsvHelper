	...
	public class ArrayConverter : IEnumerableConverter
	{
		...
		public override object ConvertFromString(string text, IReaderRow row, MemberMapData memberMapData)
		{
			...
			if (memberMapData.IsNameSet || row.Configuration.HasHeaderRecord && !memberMapData.IsIndexSet)
			{
				...
				array = (Array)row.Context.ObjectResolver.Resolve(memberMapData.Member.MemberType(), list.Count);
			...
			else
			{
				...
				array = (Array)row.Context.ObjectResolver.Resolve(memberMapData.Member.MemberType(), arraySize);
				var arrayIndex = 0;
